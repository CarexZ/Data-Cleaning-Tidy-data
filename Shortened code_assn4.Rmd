---
title: "Shortened code"
output: html_document
---
###Part1: Make merged set______________________________________________________
...
Get all the variables with mean() and std(), total 79 variables and write them in names_ms vector
```
features_list <- readLines("UCI HAR Dataset/features.txt")
mean_var <- grep("mean()", features_list) #choose all names with numbers mean()
std_var <- grep("std()", features_list) #choose all names numbers with std()
mean_std <- sort(c(mean_var, std_var)) #all names numbers for mean and std together, ordered
names_ms <- features_list[mean_std] #all NAMES with mean and std
```
...
###Part2: Create tidy set_____________________________________________________


````
library(plyr)
#-----Count one variable by subject and activity---------------------------
s <- fullset2[,.(mean(`1 tBodyAcc-mean()-X`)), by=.(subject, activity)]

#----How to move to next variable?----------------------------------------
z <- colnames(fullset2)

as.name(z[8])
```
`5 tBodyAcc-std()-Y`
Function as.name returns variable such as it can be used in column naming. But to use it we should use eval() function - at least at dplyr package!
```
#----------------------one by one--------------------------------
s <- fullset2[,.(mean(eval(z[i]))), by=.(subject, activity)]
s
```
...
```
s2 <- fullset2[,.(mean(`2 tBodyAcc-mean()-Y`)), by=.(subject, activity)]
cbind(s, s2$V1)
...
Now we have to optionsL we can create a function for creation of a tidy dataset
```
...
```
Or make it part of the script with no function. I have chosen this option, as we use this only once.
...
#--------------Option 2: No function:--------------------------
    ...
    s <- fullset2[ , mean(eval(as.name(z[4]))), by=.(subject, activity)]
        ...
        colnames(s) <- z[2:82]
```
Now the result is formally correct, but looks as a mess because of strange arrangement, so we will arrange it first by subject, then by activity
